...
function setup() {
  ...
  setInterval(calculations, 17);
  setInterval(function(){if(speed < 11) {speed += 2;}}, 10000);
...
function calculations(){
	//calculates every 17ms
	newSpaghettus();
	calcSpaghettus();
	
	function newSpaghettus(){
    if(spaghettus.y > height) {
      spaghettus = new Spaghettus();
    }
  }
  
  function calcSpaghettus(){
    spaghettus.calc();
  }
}
...
function draw() {
  ...
  function drawInfo(){
    ...
    if(level > 75) {image(info, 48, height - level + 48 /*, 700, 370*/);}
    noStroke();
  }
